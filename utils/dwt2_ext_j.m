function [a,h,v,d] = dwt2_ext_j(x,varargin)
...
if length(sx)==2
    ...
    for k = 1:3
        ...
        z = conv2(y,Lo_D1(:)','valid');
        a{k} = convdown(z,Lo_D2,dwtEXTM,sizeEXT,first,last);
        h{k} = convdown(z,Hi_D2,dwtEXTM,sizeEXT,first,last);
        z = conv2(y,Hi_D1(:)','valid');
        v{k} = convdown(z,Lo_D2,dwtEXTM,sizeEXT,first,last);
        d{k} = convdown(z,Hi_D2,dwtEXTM,sizeEXT,first,last);
    end
    a = cat(3,a{:});
    h = cat(3,h{:});
    v = cat(3,v{:});
