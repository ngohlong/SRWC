function [a,h,v,d] = dwt2_ext_i(x,varargin)
...
if ischar(varargin{1})
    [Lo_D1,Hi_D1] = wfilters(varargin{1},'d'); next = 2;
else
    Lo_D1 = varargin{1}; Hi_D1 = varargin{2};  next = 3;
end
Lo_D2 = imag(hilbert(Lo_D1)); Hi_D2 = imag(hilbert(Hi_D1));
...
if length(sx)==2
    ...
    z = conv2(y,Lo_D2(:)','valid');
    a = convdown(z,Lo_D1,dwtEXTM,sizeEXT,first,last);
    h = convdown(z,Hi_D1,dwtEXTM,sizeEXT,first,last);
    z = conv2(y,Hi_D2(:)','valid');
    v = convdown(z,Lo_D1,dwtEXTM,sizeEXT,first,last);
    d = convdown(z,Hi_D1,dwtEXTM,sizeEXT,first,last);
else
    a = cell(0,3);
    h = cell(0,3);
    v = cell(0,3);
    ...
    for k = 1:3
        ...
        z = conv2(y,Lo_D2(:)','valid');
        a{k} = convdown(z,Lo_D1,dwtEXTM,sizeEXT,first,last);
        h{k} = convdown(z,Hi_D1,dwtEXTM,sizeEXT,first,last);
        z = conv2(y,Hi_D2(:)','valid');
        v{k} = convdown(z,Lo_D1,dwtEXTM,sizeEXT,first,last);
        d{k} = convdown(z,Hi_D1,dwtEXTM,sizeEXT,first,last);
    end
    a = cat(3,a{:});
    h = cat(3,h{:});
    v = cat(3,v{:});
